...
var _path = "<%=path%>";
    ...
    /*===========================================================
     * 착한가정 자세히보기 버튼 클릭시 Function !!
     *===========================================================*/
    $(document).on("click", "#btnMore", function(e){

        gf_submitMovePage("<c:url value='/ct/cntrpt/initNicehome.do' />", "" );
	...
	$(document)
			.on(
					"click",
					"#searchZipCode , #checkPlus , #btnInsert , #arsCheck , #btnCompanyInsert ,#companyCheckPlus , #accountCheck",
					function(e) {
						e.preventDefault();
						var id = $(this).attr("id");
						if (id == "searchZipCode") {
						...
						if (id == "btnCompanyInsert") {
							...
							if (confirm("상담 신청 하시겠습니까?")) {
								var chk = $(
										"input:checkbox[name='agreeInfoCompany']")
										.is(":checked");
								var telno = $("#companyPhone1").val() + ""
										+ $("#companyPhone2").val() + ""
										+ $("#companyPhone3").val();
								$("#hidBhfCode").val(
										$("#bhfCode option:checked").val());
								$("#hidComNm").val($("#companyNm").val());
								$("#hidTelno").val(telno);
								$("#hidAgreAt").val((chk == true ? "Y" : "N"));
								$("#reqstCn").val($("#reqstCnView").val());
								$("#companyFrm > input[name=eventCode]").val(
										$("#bhfCode option:checked").attr(
												"eventCode"));
								gf_sendForm("companyFrm", url
										+ "/insertCompany.do");
							}
						}
					});
	...
	$(document).on(
			...
			"#intrstRealm",
			function(e) {
				var idx = $("#intrstRealm option").index(
						$("#intrstRealm option:selected"));
				$(".field-wrap ul li").hide();
				if (idx == 0)
					return;
	...
	function makeFormData() {
		...
		var mbtlnum = $("#phone1").val() + "-" + $("#phone2").val() + "-"
				+ $("#phone3").val();
		$("#mbtlnum").val(mbtlnum);
		...
		var cntrBgnmt = $("#cntrBgnmtY option:checked").val() + ""
				+ $("#cntrBgnmtM option:checked").val();
		...
		//개인정보 수집
		var arr = [];
		$('input[name="agreeInfo"]').each(
				function(i) {
					var chk = $("input:checkbox[name='agreeInfo']").eq(i).is(
							":checked");
					if (i == 0)
						arr = (chk == true ? "Y" : "N");
					else
						arr = arr + "-" + (chk == true ? "Y" : "N");
				});
		$("#agree_arr").val(arr);
	...
	function validator() {
		...
		if ($.trim($("#eprssNm").val()).length == 0) {
			alert("현판에 기술될 기부자명을 입력해주세요");
			$("#eprssNm").focus();
			return false;
		}

		if ($.trim($("#familyOsv").val()).length == 0) {
			alert("현판에 기술될 가훈을 입력해주세요");
			$("#familyOsv").focus();
			return false;
		}
		...
		var nowDate = today.getFullYear() + ""
				+ zeroSum(parseInt(today.getMonth()) + 1);
		var cntrAmount = uncomma($("#cntrAmount").val());
		...
		if (metpaySe == "01") {
			...
			if (moneyOnlyNumeric < 20000) {
				alert("수수료 문제로 인해 20,000원 이하 기부금은 처리할 수 없습니다.\n\n20,000원 이상 기부를 부탁드립니다.");
				$("#cntrAmount").focus();
				return false;
			}
		...
		if (metpaySe == "03") {
			if (moneyOnlyNumeric < 20000) {
				alert("수수료 문제로 인해 20,000이하 기부금은 처리할 수 없습니다.\n\n20,000원 이상 기부를 부탁드립니다.");
				$("#cntrAmount").focus();
				return false;
			}
...
<form name="form_chk" method="post">
	...
	<input type="hidden" name="result" id="result" value="N" /> <input
		type="hidden" name="companyResult" id="companyResult" value="N" /> <input
		type="hidden" name="accountResult" id="accountResult" value="N" /> <input
		type="hidden" name="arsResult" id="arsResult" value="N" />
</form>

<!-- 상담신청 폼 -->
<form id="companyFrm" name="companyFrm" method="post">
	<input type="hidden" name="bhfCode" id="hidBhfCode" /> <input
		type="hidden" name="nm" id="hidComNm" /> <input type="hidden"
		name="telno" id="hidTelno" /> <input type="hidden" name="agreAt"
		id="hidAgreAt" /> <input type="hidden" name="iemCode" id=""
		value="02" /> <input type="hidden" name="eventCode" id="" value="" />
	<input type="hidden" name="reqstManageTy" id="" value="0004" />
	<!-- HP0025 -->
...
<div class="contents-area">
	<div class="contents-hd">
		<h3>
			<strong id="eventNm">착한가정</strong><span>당신의 나눔이 모두의 행복입니다. 나눔의
				힘을 믿어보세요.</span>
		</h3>
	...
	<div class="seo-box">
		...
		<form id="frm" name="frm" method="post" enctype="multipart/form-data"
			action="<c:url value='/ct/fdrmcntr/insert.do' />">
			...
			<!-- 영수증 신청시 주민번호 -->
			<input type=hidden name="cntrBgnmt" id="cntrBgnmt" />
			<!-- 기부시작월 -->
			<input type=hidden name="cntrEndmt" id="cntrEndmt" />
			<!-- 기부종료월 -->
			<input type=hidden name="cntrTransfrDe" id="cntrTransfrDe" />
			<!-- 결제 , 이체지정일 -->
			<input type=hidden name="cardExpdate" id="cardExpdate" />
			<!-- 카드유효기간 -->
			<input type=hidden name="chargerCnfirmAt" id="chargerCnfirmAt"
				value="N" />
			<!-- 담당자 확인여부 -->
			<input type=hidden name="sign" id="sign" />
			<!-- 생년월일 -->
			<input type=hidden name="agree_arr" id="agree_arr" /> <input
				type=hidden name="timestamp" id="timestamp" value="<%=timestamp%>">
			...
			<div class="area-type-a type8">
				...
				<p class="area-type-tit color-type1 mgt-xxl mgb-n">
					나눔의 씨앗과 우리 자녀가 함께 자라납니다.<br>나눔은 우리 가족 깊게 뿌리내린 가풍입니다.
				</p>
				<p>
					나눔콜센터(080-890-1212)통해서 빠르고 친절하게 <br>가입상담 받으실 수 있습니다.
				</p>
			</div>
			<!-- E : 2019-10-28 modify -->
			...
			<div class="donation-guide">
				<dl>
					...
					<dd class="donation-guide-item n1">
						<div>
							신용카드 기부 <strong>15일 이후</strong>
						</div>
					</dd>
					<dd class="donation-guide-item n2">
						<div>
							정기이체 기부 <strong>15일 이후</strong>
						</div>
					</dd>
				</dl>
				<p class="donation-guide-txt">
					기부시 <b>후원금_KG 이니시스, KG 이니시스</b>로 표시되어 청구됩니다.
				</p>
			</div>
								...
								<td>
									<div class="radio-group">
										...
										<span class="radio"> <input type="radio"
											name="metpaySe" id="metpaySe2" value="03"
											onclick="radioEvent('metpaySe');" checked> <label
											for="metpaySe2"><span>정기이체</span></label>
										</span>
									...
									</div> <!-- 						<p class="donation-guide-txt">온라인계좌이체 기부시 <b>후원금_KG 이니시스, KG 이니시스</b>로 표시되어 청구됩니다.</p> -->
			...
			<div class="tb-type_a">
				<table summary="희망지역/기부금액 입력">
					<caption>희망지역/기부금액 입력해주세요</caption>
					<colgroup>
						<col class="col-xs">
						<col class="col-auto">
					</colgroup>
					<tbody>
						<tr>
							<th scope="row"><label for="eventCode">희망지역<span
									class="required" aria-label="필수">*</span></label></th>
							<td class="hope-area">
								<div class="ui-select form-w01">
									<select class="sel_type" id="eventCode" name="eventCode"
										title="희망지역 선택"></select>
								</div>
								<p class="donation-guide-txt">
									사회복지공동모금회법에 의해 각 지역에서 모금된 성금은 <b>해당지역의 사회복지</b>를 위해 쓰여집니다.
								</p>
							</td>
						</tr>
						<tr>
							...
							<td>
								<div class="ui-togglebtn type-sum">
									...
									<input type="text" class="inp-base ui-inpcancel" name="cntrAmount" id="cntrAmount" title="금액직접입력"  placeholder="직접입력" readonly
										numberOnly maxlength="9"><span class="txt-mark">원</span>
								</div>
							</td>
						</tr>
					</tbody>
			...
			<div class="tb-type_a">
				<table summary="기부자 정보 입력">
					...
					<tbody>
                        <tr>
                            <th scope="row"><label for="phone1">휴대폰 번호<span
                                    class="required" aria-label="필수">*</span>
                            </label></th>
                            <td>
                                <div class="phone-area">
                                    <input type="text" class="inp-base" title="휴대폰 앞자리"
                                        name="phone1" id="phone1" readonly value=""> <span
                                        class="txt-mark">-</span> <input type="text" class="inp-base"
                                        title="휴대폰 중간자리" name="phone2" id="phone2" readonly value="">
                                    <span class="txt-mark">-</span> <input type="text"
                                        class="inp-base" title="휴대폰 뒷자리" name="phone3" id="phone3"
                                        readonly value="">
                                    <button type="button" class="btn-base-imp small" id="checkPlus" title="새창 열림">인증</button>
                                </div>
                                <p class="donation-guide-txt">
                                    - NICE 평가정보에서 인증 받은 휴대폰 번호를 사용하고 있습니다.<br> - 기부자 본인을 확인하기
                                    위해서 기부자님의 본인인증이 반드시 필요합니다.<br> - 본인인증시 반복적으로 오류가 발생하면
                                    NICE기술지원(1600-1522)로 전화부탁드립니다.
                                </p>
                            </td>
                        </tr>

						<tr>
							<th scope="row"><label for="nm">이름<span
									class="required" aria-label="필수">*</span></label></th>
							<td><input type="text" class="form-w01" name="nm" id="nm"
								title="기부자 이름" readonly="readonly"></td>
						</tr>
						<tr>
							<th scope="row"><label for="yy">생년월일<span
									class="required" aria-label="필수">*</span></label></th>
							<td>
								<div class="day-form">
						...
						<tr>
							<th scope="row"><label for="eprssNm">현판에 기술될 <br>기부자명<span
									class="required" aria-label="필수">*</span>
							</label></th>
							<td><input type="text" class="inp-base form-w01"
								placeholder="현판에 기술될 기부자명" title="현판에 기술될 기부자명"  id="eprssNm"
								name="eprssNm"></td>
						</tr>
						<tr>
							<th scope="row"><label for="cckfunr01_i16">현판에 기술될 <br>가훈<span
									class="required" aria-label="필수">*</span>
							</label></th>
							<td><input type="text" class="inp-base form-w01"
								title="현판에 기술될 가훈" id="familyOsv" name="familyOsv"></td>
						</tr>
					</tbody>
				</table>
			</div>
			...
			<div class="tb-type_a">
				<table summary="기부방식 정보설정">
					...
					<tbody>
						<tr>
							<th scope="row"><label for="cntrBgnmtY">기부시작월<span
									class="required" aria-label="필수">*</span></label></th>
							<td>
								<div class="day-form">
									<div class="day-form-row">
						...
						<tr class="cardLayoutDe">
							<th scope="row"><label for="cntrTransfrDe1">결제일 지정<span
									class="required" aria-label="필수">*</span>
							</label></th>
							<td>
								<div class="day-form">
						...
						<tr class="cardLayout">
							<th scope="row"><label for="cardNo">카드번호<span
									class="required" aria-label="필수">*</span></label></th>
							<td><input type="text" class="form-w01" id="cardNo"
								name="cardNo" title="카드번호" maxlength="16" numberOnly></td>
						</tr>
						<tr class="cardLayout">
							<th scope="row"><label for="cckfunr01_s8">카드유효기간<span
									class="required" aria-label="필수">*</span>
							</label></th>
							<td>
								<div class="day-form">
									<div class="day-form-row">
										<div class="ui-select sel_small2">
											<select class="sel_type" id="cardExpdateY"
												name="cardExpdateY" title="카드유효기간 년 선택"></select>
										</div>
										<span class="day-txt">년</span>
									</div>
									<div class="day-form-row">
										<div class="ui-select sel_small2">
											<select class="sel_type" id="cardExpdateM"
												name="cardExpdateM" title="카드유효기간 월 선택"></select>
										</div>
										<span class="day-txt">월</span>
									</div>
						...
						<tr class="onlineLayout">
							<th scope="row"><label for="cckfunr01_s11">이체일 지정<span
									class="required" aria-label="필수">*</span></label></th>
							<td>
								<div class="day-form">
									<div class="day-form-row">
									...
									<p class="donation-guide-txt">본인 명의 통장만 가능하며 2주후부터 매월 지정하신
										날짜에 이체가 시작됩니다.</p>
								</div>
							</td>
						</tr>
						<tr class="onlineLayout">
							<th scope="row"><label for="acnutBank">계좌은행<span
									class="required" aria-label="필수">*</span></label></th>
							<td>
								<div class="ui-select form-w01">
									<select class="sel_type" id="acnutBank" name="acnutBank"
										title="계좌은행 선택"></select>
								</div>
								<p class="donation-guide-txt">- CMA 통장은 자동이체가 되지 않으니 양해
									바랍니다.</p>
							</td>
						</tr>
						<tr class="onlineLayout">
							<th scope="row"><label for="acnutno">계좌번호<span
									class="required" aria-label="필수">*</span></label></th>
							<td><input type="text" class="form-w01" name="acnutno" id="acnutno" placeholder="숫자만 입력" title="계좌번호" maxlength="20" numberOnly>
								<button type="button" class="btn-base-imp small" id="accountCheck" title="계좌인증">계좌인증</button></td>
						</tr>
						<tr class="onlineLayout">
							<th scope="row"><label for="dpstr">예금주<span
									class="required" aria-label="필수">*</span></label></th>
							<td><input type="text" class="form-w01" name="dpstr"
								id="dpstr" title="예금주"></td>
						</tr>
						<tr class="onlineLayout">
							<th scope="row"><label for="dpstr">자동이체<br>
									추가인증<span class="required" aria-label="필수">*</span></label></th>
							<td>
								<!-- @190921 추가 -->
								<div id="directDebitAddCert" class="onlineLayout">
									<p class="donation-guide-txt">온라인 자동이체의 경우 전자금융거래법 제15조 및
										동법 시행령 제 10조에 의겨해 출금동의 인증을 받도록 되었습니다.</p>
									<div class="radio-group">
										<span class="radio"> <input type="radio"
											name="certification" id="certification1" value="pad"
											onclick="radioEvent('certification');" checked> <label
											for="certification1"><span>일반전자서명 인증</span></label>
										</span> <span class="radio"> <input type="radio"
											name="certification" id="certification2" value="ars"
											onclick="radioEvent('certification');"> <label
											for="certification2"><span>ARS 인증</span></label>
										</span> <span class="radio"> <input type="radio"
											name="certification" id="certification3" value="agree"
											onclick="radioEvent('certification');"> <label
											for="certification3"><span>출금의뢰동의서</span></label>
										</span>
									</div>
									...
									<div class="addcert n1" id="signature-pad">
										...
										<p class="donation-guide-txt">마우스를 위 영역에 올린 후 클릭한 상태에서
											서명해주세요.</p>
										<button type="button" class="btn-base-imp small mgt-xxs"
											data-action="clear">다시 서명하기</button>
										<!-- @190921 수정 -->
									</div>
									...
									<div class="addcert n2" style="display: none" id="ars">
										...
										<div class="phone-area">
											<input type="text" class="inp-base" title="휴대폰 앞자리"
												id="arsPhone1" maxlength="3" numberOnly value=""> <span
												class="txt-mark">-</span> <input type="text"
												class="inp-base" title="휴대폰 중간자리" id="arsPhone2"
												maxlength="4" numberOnly value=""> <span
												class="txt-mark">-</span> <input type="text"
												class="inp-base" title="휴대폰 뒷자리" id="arsPhone3"
												maxlength="4" numberOnly value="">
											<button type="button" class="btn-base-imp small"
												id="arsCheck">ARS 인증전화 받기</button>
										</div>
										<p class="donation-guide-txt">
											- 휴대폰번호로 전화가 오면 안내에 따라 인증을 진행하세요.<br> - 전화 인증이 끝나면 아래
											추가정보를 입력하세요.
										</p>
									</div>
								</div>
								...
								<div class="addcert n3" style="display: none" id="agree">
									<p class="donation-guide-txt mgt-xs mgb-xs">
										<span>출금의뢰동의서 양식을 다운받으셔서 작성후 다시 업로드 해주세요. </span> <a href="#"
											class="btn-base-bline small mgl-xs"
											onclick="gf_genFileExport(this);">출금의뢰동의서 다운로드</a> <input
											id="filePath" name="filePath" type="hidden"
											value="<c:out value='${downPath}' escapeXml='false' />" /> <input
											id="phycFileNm" name="phycFileNm" type="hidden"
											value="agree.hwp" /> <input id="logcFileNm"
											name="logcFileNm" type="hidden" value="agree.hwp" />
									</p>

									<div class="fileupload-area">
										<input type="text" class="inp-base" title="출금의로동의서 업로드"
											id="txt" readonly="readonly"> <input type="file"
											size="30" id="file" name="attachFile1" title="파일 찾아보기" style="display: none;" 
											onchange="fileOnchangeEvent(this);" />
										<button type="button" class="btn-base small"
											onclick="$('#file').click();">찾아보기</button>
									</div>
						...
						<tr>
							<th scope="row"><label for="receiptYn1">영수증 신청 여부<span
									class="required" aria-label="필수">*</span>
							</label></th>
							<td>
								<div class="radio-group">
									<span class="radio"> <input type="radio"
										name="rciptReqstAt" id="receiptYn1" value="Y"
										onclick="radioEvent('rciptReqstAt');" checked> <label
										for="receiptYn1"><span>신청</span></label>
									</span> <span class="radio"> <input type="radio"
										name="rciptReqstAt" id="receiptYn2" value="N"
										onclick="radioEvent('rciptReqstAt');"> <label
										for="receiptYn2"><span>미신청</span></label>
									</span>
								</div>
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="jumin1">영수증 신청 <br>주민등록번호<span
									class="required" aria-label="필수">*</span></label></th>
							<td>
								<div class="jumin-area">
									<input type="text" class="inp-base" title="주민등록번호 앞자리"
										name="jumin1" id="jumin1" numberOnly maxlength="6"> <span
										class="txt-mark">-</span> <input type="password"
										class="inp-base" title="주민등록번호 뒷자리" name="jumin2" id="jumin2"
										numberOnly maxlength="7">

								</div>
								<p class="donation-guide-txt">
									- 기부금영수증발급을 위한 필수 정보입니다.<br>- 개인정보보호법 제2조, 사회복지공동모금회법
									제18조, 소득세법 제160조의3, 소득세법시행령 제208조의 3
								</p>
							</td>
						</tr>
						<tr>
							...
							<td>
								<div class="post-code">
									<input type="text" class="form-w02" name="zip" id="zip"
										readonly title="우편번호">
									<button type="button" class="btn-base-imp small"
										id="searchZipCode" title="새창 열림">우편번호 검색</button>
								</div>
							</td>
						</tr>
						<tr>
							<th scope="row"><label for="adres1"> 주소<span
									class="required" aria-label="필수">*</span>
							</label></th>
							<td><input type="text" class="form-w01" readonly title="주소"
								name="adres1" id="adres1"></td>
						</tr>
						<tr>
							<th scope="row">
								<label for="adres2">상세주소<span
									class="required" aria-label="필수">*</span></label>

							</th>
							<td><input type="text" class="form-w01" title="상세주소"
								name="adres2" id="adres2"></td>
						</tr>
						<tr>
							<th scope="row">
							...
							<td>
								<div class="email-area">
									<input type="text" class="in-w03" title="이메일 아이디" name="email1"
										id="email1" style="ime-mode: disable;"> <span
										class="txt-mark">@</span> <input type="text" class="in-w03"
										title="이메일 도메인주소" name="email2" id="email2"
			...
			<div class="tb-type_a">
				<table summary="추가 정보 입력">
					<caption>추가 정보 입력폼입니다.</caption>
					<colgroup>
						<col class="col-xs">
						<col class="col-auto">
					</colgroup>
					<tbody>
						<tr>
							...
							<td>
								<div class="ui-select form-w01">
									<select class="sel_type" id="intrstRealm" name="intrstRealm"
										title="관심분야 선택">
								...
								<div class="field-wrap">
									<ul>
										...
										<li class="n2"><span class="hide">2. 영양 및
												급식지원/기아종식</span></li>
										...
										<li class="n4"><span class="hide">4. 교육 및 자립역량 강화
												...
												<li class="n7"><span class="hide">7. 모두를 위한 깨끗한
														에너지</span></li>
												<li class="n8"><span class="hide">8. 양질의 일자리만들기</span></li>
												<li class="n9"><span class="hide">9. 적정기술과 정보기술
														격차 해소 지원</span></li>
												<li class="n10"><span class="hide">10. 사회적 배제
														감소와 불평등 완화</span></li>
												<li class="n11"><span class="hide">11. 지속가능한
														지역사회 인프라 구축</span></li>
												<li class="n12"><span class="hide">12. 지속가능한 생산과
														소비</span></li>
												...
												<li class="n16"><span class="hide">16. 사회적 약자의
														권리증진</span></li>
		...
		<div class="terms-group">
			<span class="checkbox"> <input type="checkbox"
				name="agreeInfo" id="agreeInfo"> <label for="agreeInfo"><span>개인정보
						수집 및 이용에 동의 합니다.</span></label>
			</span>
...
본인은 모금회가 「개인정보 보호법」제15조제1항 및 제24조제1항제1호에 의거, 다음과 같이 본인의 개인정보 및 고유식별정보(이하 “개인정보”라 함)를 수집ㆍ이용하는 것에 대하여 동의합니다.
  ...
  라. 개인정보의 보유 및 이용기간 : 1.주민등록번호 : 기부금 입금일을 기준으로 소득공제가 실시되는 기간동안, 2.기타 : 모금회가 정한 보존기간동안</textarea>
			<span class="checkbox"> <input type="checkbox"
				name="agreeInfo" id="agreeInfo1"> <label for="agreeInfo1"><span>개인정보
						제 3자 제공에 동의 합니다.</span></label>
			</span>
  ...
  라. 개인정보를 제공받는 자의 보유 및 이용기간 :
     ...
     기타 : 모금회가 정한 보존기간동안</textarea>
			<div class="btn-area-r">
				<button type="button" class="btn-base-bline small"
					onclick="javascript:goFootPrivacy();">개인정보처리방침</button>
			</div>
		</div>
		<!-- //동의 -->
		...
		<!-- 기부하기 -->
		<div class="btn-area">
			<button type="button" class="btn-base-bline fl-item-l"
				id="btnCounsel">기부상담하기</button>
			<button type="button" class="btn-base-imp" id="btnInsert">기부하기</button>
			<button type="button" class="btn-base-line fl-item-r"  id="btnMore">착한가정 소개 자세히보기</button>
		</div>
		<!-- 기부하기 -->
		...
		<div id="" class="sb-box">
			...
			<div class="sb-box-inner">
				...
				<p class="sb-contents-desc">기부상담을 위해 상담받으실 분 성함과 연락처를 남겨주시면
					빠른시간안에 연락드리도록 하겠습니다.</p>
				<div class="sb-content">
					<div class="sb-content-tbl">
						<table summary="기부상담신청">
							<caption>기부상담신청폼입니다.</caption>
							<colgroup>
								<col class="col-l">
								<col class="col-auto">
							</colgroup>
							<tbody>
								<tr>
									<th scope="row"><label for="bhfCode">상담지역</label></th>
									<td>
										<div class="ui-select w-full">
											<select class="sel_type" id="bhfCode" name="bhfCode"
												title="상담지역 선택"></select>
										</div>
									</td>
								</tr>
								<tr>
									<th scope="row"><label for="companyNm">성명</label></th>
									<td><input type="text" class="inp-base w-full"
										id="companyNm" title="성명" readonly="readonly"></td>
								</tr>
								<tr>
									<th scope="row"><label for="companyPhone1">휴대폰번호<span
											class="required" aria-label="필수">*</span></label></th>
									<td class="phone-area">
										<div class="table-cell ps-box">
											<span> <input type="text" class="inp-base"
												title="휴대폰 앞자리" id="companyPhone1" readonly="readonly"
												numberOnly value="">
											</span> <span> <input type="text" class="inp-base"
												title="휴대폰 중간자리" id="companyPhone2" readonly="readonly"
												numberOnly value="">
											</span> <span> <input type="text" class="inp-base"
												title="휴대폰 뒷자리" id="companyPhone3" readonly="readonly"
												numberOnly value="">
											</span>
											<button type="button" class="btn-base-imp small"
												id="companyCheckPlus">인증</button>
										</div>

										<p class="table-cell">
											- NICE 평가정보에서 인증 받은 휴대폰 번호를 사용하고 있습니다.<br> - 본인인증시 반복적으로
											오류가 발생하면 NICE기술지원(1600-1522)로 전화부탁드립니다.
										</p>
                        ...
                        <div class="table-cell privacy-area">
                            ...
                            <div class="tb-type_d mgt-xxs">
                                <div class="wide-scroll">
                                    <table summary="개인정보 수집‧이용에 관한 정보">
                                        <caption>개인정보 수집‧이용에 관한 정보입니다.</caption>
                                        <colgroup>
		...
		<div class="donation-guide">
			<dl>
				<dt class="donation-guide-tit">연말정산을 위한 기부금 영수증 발급이 가능한 시점이
					결제수단마다 다르니 참고하세요!</dt>

				<dd class="donation-guide-item n3">
					<div>
						신용카드 기부<br>온라인계좌이체 기부 <strong>12월 16일 이전</strong>
					</div>
				</dd>
				<dd class="donation-guide-item n4">
					<div>
						휴대폰 기부 <strong>10월 이전</strong>
					</div>
				</dd>
				<dd class="donation-guide-item n5">
					<div>
						무통장입금 기부 <strong>12월 31일 이전</strong>
					</div>
				</dd>
			</dl>
			<p class="donation-guide-txt">
		...
		<div class="donation-bn">
			<a href="#"> <img src="/common/img/contents/bn_donation.png"
				srcset="/common/img/contents/bn_donation@2x.png 2x"
				alt="기부관련 상담을 원하시나요? 나눔콜센터 080-890-1212">
			</a>
		</div>
...
<script>
	...
	(function($, win, doc, undefined) {
		$(doc).ready(function() {
			...
			//메뉴는 00 부터 시작
			// 	    $plugins.common.init({
			// 			menuid: '01_00_01' //1depth_2depth_3depth
			// 		});
	...
	function makeSelectBox() {
		...
		el = "";
		for (var i = 1; i <= 12; i++) {
			el += "<option value='" + zeroSum(i) + "'>" + zeroSum(i)
					+ "</option>";
		}
		$("#mm").html(el);
	...
	function selectSelListAfter(data) {
		...
		$(data.bankList).each(
				function(i, item) {
					el += "<option value='" + item.cmmnCode + "'>"
							+ item.cmmnCodeNm + "</option>";
				});
		$("#acnutBank").html(el);
		...
		$(data.codeList0)
				.each(
						function(i, item) {
							el += "<option value='" + item.eventCode + "' bhfCode='"+item.bhfCode+"'>"
									+ item.bhfCodeNm + "</option>";
						});
		$("#eventCode").html(el);
		$("select[name='eventCode'] option[bhfCode='" + __branchCd + "']")
				.prop("selected", true);
		...
		$(data.codeList1).each(
				function(i, item) {
					el += "<option value='"+item.bhfCode+"'>" + item.bhfCodeNm
							+ "</option>";
				});
		$("#bhfCode").html(el);
		$("select[name='bhfCode'] option[value='" + __branchCd + "']").prop(
				"selected", true);
	...
	function fileOnchangeEvent(obj) {
		...
		el = '<li><span>'
				+ fileName
				+ '</span><button type="button" class="btn-del">삭제</button></li>';
