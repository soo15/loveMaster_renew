		...
		* - $plugins.uiSelectAct({ option });
		...
		function createUiSelect(opt) {
			...
			for (i = 0; i < len; i++) {
				...
				var html_btn = '<button type="button" class="ui-select-btn" id="' + sel_id + '_inp" role="combobox" aria-autocomplete="list" aria-owns="' + list_id + '" aria-haspopup="true" aria-expanded="false" aria-activedescendant="' + opt_id_selected + '" data-n="' + sel_n + '" data-id="' + sel_id + '"';
				!!vchecktype
			...
			function optOpen(t) {
				...
				customscroll
				? $ui.uiScrollBar({ id: _$wrap.attr('id'), top: _$wrap.find('.selected').index() * _$wrap.find('.ui-select-opt').outerHeight() })
				: '';
			}
		...
		function createUiSelectAct(opt) {
			...
			if ($opt.eq(current).val() === 'direct') {
				$uisel.find('.ui-select-btn').prop('readonly', false).text('').focus();
			} else {
				...
				$inp.eq(i).off('keyup.inpcancel focus.inpcancel').on('keyup.inpcancel focus.inpcancel', function () {
					...
					if (_$this.val() === '') {
						...
						!!$('.ui-btn-cancel[data-id="' + _$this.attr('id') + '"]').length ? '' :
							_$this.after('<button type="button" class="ui-btn-cancel" data-id="' + _$this.attr('id') + '" tabindex="-1" arai-hidden="true"><span>입력내용 지우기</span></button>');
					}
		...
		$ui.modal = {
			system: function (opt) {
				...
				var html_system = '';
					...
					html_system += btn_b !== false ? '<button type="button" class="btn-txt" id="modalSystemBtn2">'+ btn_b +'</button>' : '';
					html_system += '<button type="button" class="btn-check" id="modalSystemBtn1">'+ btn_a +'</button>';
					html_system += '</div>';
					html_system += '</div>';
					html_system += '</section>';
